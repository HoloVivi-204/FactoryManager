...
export const API_BASE_URL = import.meta.env.VITE_API_URL ?? 'http://localhost:8080/factory-management/api/v1'
...
function successMessage(path: string, method: Method, serverMessage?: string) {
  ...
  if (path.includes('/leave-requests/') && path.endsWith('/decision')) return 'Đã xử lý đơn nghỉ phép.'
  if (path.includes('/overtime-requests/') && path.endsWith('/decision')) return 'Đã xử lý đăng ký tăng ca.'
  ...
  if (path.includes('/hr/schedules')) return method === 'DELETE' ? 'Đã ngừng lịch làm việc.' : 'Đã lưu lịch làm việc.'
  ...
  if (path.includes('/notifications/') && path.endsWith('/read')) return 'Đã đánh dấu thông báo là đã đọc.'
  if (path.includes('/maintenance/requests/') && path.endsWith('/status')) return 'Đã cập nhật trạng thái yêu cầu bảo trì.'
  if (path === '/maintenance/requests') return 'Đã gửi yêu cầu bảo trì.'
  if (path.includes('/maintenance/schedules')) return method === 'DELETE' ? 'Đã ngừng lịch bảo trì.' : 'Đã lưu lịch bảo trì.'
  if (path.includes('/maintenance/work-orders/') && path.endsWith('/status')) return 'Đã cập nhật trạng thái phiếu bảo trì.'
  if (path.includes('/maintenance/work-orders/') && path.includes('/parts')) return method === 'DELETE' ? 'Đã bỏ vật tư khỏi phiếu bảo trì.' : 'Đã thêm vật tư vào phiếu bảo trì.'
...
export async function uploadFile<T>(path: string, file: File, notify = false): Promise<T> {
  ...
  try {
    ...
    return fail(response.status === 413
      ? 'File Excel vượt quá giới hạn 10 MB.'
      : `Máy chủ không xử lý được file (HTTP ${response.status}).`, notify)
  }
  if (response.status === 401) {
    TokenManager.clear()
    window.dispatchEvent(new Event('factory:unauthorized'))
  }
  if (!response.ok || body.code !== 1000) return fail(body.message ?? 'Không thể xử lý file Excel.', notify)
