...
- Một class, endpoint hoặc màn hình tồn tại chưa đủ để kết luận một yêu cầu đã hoàn tất.
  ...
  [factory-performance-feature-matrix.csv](factory-performance-feature-matrix.csv).

## 2. Nguồn chuẩn và thứ tự ưu tiên

1. [PRD v2.5](PRD_He_thong_hieu_suat_nha_may.md) quyết định nhu cầu người dùng,
   phạm vi, business rule và acceptance criteria.
2. [TDD v1.5](TDD_He_thong_hieu_suat_nha_may.md) quyết định hướng triển khai kỹ thuật.
3. Quyết định đã duyệt trong `DECISIONS.md` làm rõ các điểm PRD/TDD còn mở.
...
- Backend là Spring Boot, Spring Security, JPA và PostgreSQL; frontend là React, TypeScript và
  Vite. Xem [pom.xml](factory-management/pom.xml) và
...
- JWT login/refresh/logout, role, explicit data scope và nhiều kiểm tra `@PreAuthorize` đã có.
  Xem [SecurityConfig.java](factory-management/src/main/java/com/factory/management/config/SecurityConfig.java)
  và [AuthorizationScope.java](factory-management/src/main/java/com/factory/management/security/AuthorizationScope.java).
...
- Form staging, các detail downtime/chất lượng và approval thành production report đã có.
  Xem [ProductionReportStagingServiceImpl.java](factory-management/src/main/java/com/factory/management/service/ServiceImpl/ProductionReportStagingServiceImpl.java).
- Import Excel có template, preview và hiển thị lỗi, nhưng hiện dùng mapping cột cố định và chỉ
  ...
  Xem [StagingReportExcelService.java](factory-management/src/main/java/com/factory/management/service/ServiceImpl/StagingReportExcelService.java).
...
- Dashboard hiện tính trực tiếp từ production reports; chưa có KPI Dictionary, snapshot,
  ...
  Xem [ProductionReportServiceImpl.java](factory-management/src/main/java/com/factory/management/service/ServiceImpl/ProductionReportServiceImpl.java).
...
- Cấu hình phát triển còn fallback JWT/admin password trong
  [application.yaml](factory-management/src/main/resources/application.yaml), và schema đang dựa
...
- [Backlog vertical slices](Backlog_He_thong_hieu_suat_nha_may.md)
- [Cây domain và stable IDs](content-skeleton.yaml)
- [Feature matrix](factory-performance-feature-matrix.csv)
- `ARCHITECTURE.md`
- `API_Contracts.md`
- `DECISIONS.md`
- `PLANS.md`
