...
export default function TeamLeaderDashboard({ reports }: { reports: StagingReport[] }) {
  ...
  const fromDate = startDate(days)

  const filtered = useMemo(
    () => reports.filter((row) => row.reportDate >= fromDate && row.reportDate <= today),
    [reports, fromDate, today],
  )
  ...
  const metrics = useMemo(() => {
    ...
    const good = filtered.reduce(
      (total, row) => total + Number(row.goodQuantity ?? row.actualQuantity - row.defectQuantity), 0,
    )
    const downtime = sum(filtered, 'downtimeMinutes')
    const working = sum(filtered, 'workingMinutes')
    return {
      planned, actual, good, defect, downtime, working,
      gap: actual - planned,
  ...
  const workflow = useMemo(() => ({
    draft: reports.filter((row) => row.status === 'DRAFT').length,
    change: reports.filter((row) => row.status === 'CHANGE_REQUESTED').length,
    submitted: reports.filter((row) => row.status === 'SUBMITTED').length,
    completed: reports.filter((row) => row.status === 'APPROVED' || row.status === 'LOCKED').length,
  }), [reports])
  ...
  const people = useMemo(() => {
    ...
    return {
      ...
      overtime: details.employees.reduce((total, row) => total + Number(row.overtimeMinutes ?? 0), 0),
      working: details.employees.reduce((total, row) => total + Number(row.workingMinutes ?? 0), 0),
    }
  }, [details.employees])

  const trend = useMemo(() => {
    const values = new Map<string, { planned: number; actual: number; good: number; defect: number; downtime: number }>()
    filtered.forEach((row) => {
      const current = values.get(row.reportDate) ?? { planned: 0, actual: 0, good: 0, defect: 0, downtime: 0 }
  ...
  const materialTypes = useMemo(
    () => group(details.materials, (row) => issueLabel(String(row.issueType ?? 'OTHER')), () => 1, ' lần'),
    [details.materials],
  )
  ...
  return (
    <section className="team-dashboard">
      <div className="team-dashboard-heading">
        <div><h3>Dashboard vận hành của tổ</h3><p>Từ {dateLabel(fromDate)} đến {dateLabel(today)} · {filtered.length} báo cáo</p></div>
        <div className="period-switch">
          {[{ value: 1, label: 'Hôm nay' }, { value: 7, label: '7 ngày' }, { value: 30, label: '30 ngày' }].map((item) => (
            <button key={item.value} className={days === item.value ? 'active' : ''} onClick={() => setDays(item.value)}>{item.label}</button>
          ))}
        </div>
      </div>
            ...
            {trend.map(([date, value]) => {
              ...
              return (
                <div className="team-trend-row rich" key={date}>
                  ...
                  <div className="team-trend-track"><i style={{ width: `${Math.min(rate, 100)}%` }} /></div>
                  <b>{nf.format(value.actual)} / {nf.format(value.planned)}</b>
                  <small>{percentage(rate)}</small>
                  <em>{nf.format(value.good)} tốt · {nf.format(value.defect)} lỗi · {nf.format(value.downtime)} phút dừng</em>
                </div>
              )
            })}
            {trend.length === 0 && <p className="dashboard-empty">Chưa có báo cáo trong khoảng thời gian này.</p>}
          </div>
        </Panel>
        <Panel title="Việc cần xử lý">
          <div className="team-alerts">
            {alerts.map((alert, index) => <article className={alert.tone} key={`${alert.title}-${index}`}><span /><div><b>{alert.title}</b><p>{alert.text}</p></div></article>)}
          </div>
        </Panel>
      </div>
          ...
          <table>
            ...
            <tbody>
              {detailReports.map((row) => {
                ...
                const defectRate = row.actualQuantity > 0 ? row.defectQuantity * 100 / row.actualQuantity : 0
                return <tr key={row.id}>
                  <td><b>{dateLabel(row.reportDate)}</b></td><td>{row.shiftName}</td><td>{row.machineCode}</td>
                  <td>{nf.format(row.plannedQuantity)}</td><td>{nf.format(row.actualQuantity)}</td>
              ...
              {detailReports.length === 0 && <tr><td colSpan={14} className="empty">Chưa có báo cáo trong khoảng thời gian này</td></tr>}
            </tbody>
          </table>
        </div>
      </Panel>
      ...
      <Panel title="Trạng thái luồng báo cáo">
        <div className="team-workflow">
          <div><b>{workflow.draft}</b><span>Đang nhập</span><small>Cần hoàn thiện</small></div>
          <div><b>{workflow.change}</b><span>Cần sửa</span><small>Theo góp ý quản lý</small></div>
          <div><b>{workflow.submitted}</b><span>Chờ duyệt</span><small>Đã gửi quản lý</small></div>
          <div><b>{workflow.completed}</b><span>Đã duyệt/chốt</span><small>Dữ liệu chính thức</small></div>
        </div>
      </Panel>
    </section>
  )
...
function InsightCard({ tone, label, main, rows, interpretation }: {
  tone: string
  label: string
  main: string
  rows: [string, string][]
  ...
  return <article className={`team-insight ${tone}`}>
    <span className="team-insight-label">{label}</span><strong>{main}</strong>
    <div>{rows.map(([name, value]) => <p key={name}><span>{name}</span><b>{value}</b></p>)}</div>
...
function Breakdown({ title, rows, empty }: { title: string; rows: { label: string; value: string; raw: number }[]; empty: string }) {
  ...
  return <section><h4>{title}</h4>{rows.slice(0, 5).map((row) => <div className="breakdown-row" key={row.label}>
    <p><span>{row.label}</span><b>{row.value}</b></p><i><span style={{ width: `${row.raw * 100 / max}%` }} /></i>
  </div>)}{rows.length === 0 && <p className="breakdown-empty">{empty}</p>}</section>
}
