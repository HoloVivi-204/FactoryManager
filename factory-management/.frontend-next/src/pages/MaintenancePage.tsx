...
function MaintenanceRequestsView({ canManage, canCreate }: { canManage: boolean; canCreate: boolean }) {
  const defaults = { machineId: '', teamId: '', status: '', priority: '' }
  const [filters, setFilters] = useState(defaults)
  const [applied, setApplied] = useState(defaults)
  ...
  const createFields: FieldSpec[] = [
    ...
    { key: 'sourceDowntimeStagingId', label: 'ID downtime staging', type: 'number', min: 1, hint: 'Không bắt buộc; nếu có phải cùng máy.' },
  ...
  return (
    <>
      ...
      {createOpen && (
        <EditorModal title="Tạo yêu cầu bảo trì" subtitle="Yêu cầu mới luôn bắt đầu ở trạng thái Mới tạo." fields={createFields} values={createForm} onChange={setCreateForm} busy={busy} error={modalError} onClose={() => setCreateOpen(false)} onSubmit={createRequest} />
      )}
      {statusTarget && (
        <EditorModal
