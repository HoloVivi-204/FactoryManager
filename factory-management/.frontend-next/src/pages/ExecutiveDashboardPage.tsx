    ...
    <>
      <div className="page-title executive-title">
        <div>
          ...
          <p>
            Xu hướng tài chính, sản xuất, chất lượng và các vấn đề cần Ban giám đốc quyết định.
          </p>
        </div>
        <div className="executive-refresh">
          <small>Cập nhật gần nhất</small>
          <b>{dateTime(data?.generatedAt)}</b>
          <button type="button" onClick={api.reload} disabled={api.loading}>Làm mới</button>
        </div>
      </div>
          ...
          <label className="executive-factory-filter">
            ...
            >
              ...
              {factoryOptions.map((factory) => (
                <option key={factory.factoryId} value={factory.factoryId}>{factory.factoryName}</option>
              ))}
            </select>
          </label>
      ...
      {!api.loading && !api.error && data && (
        <>
          <div className="executive-scope-note">
            <span>Phạm vi: <b>{data.scopeName}</b></span>
            <span>Kỳ báo cáo: <b>{data.fromDate} — {data.toDate}</b></span>
            <span>Nhịp xu hướng: <b>{data.trendGranularity === 'MONTH' ? 'Theo tháng' : 'Theo ngày'}</b></span>
          </div>
          ...
          {!hasData ? (
            <section className="executive-empty">
              ...
              <p>Hãy đổi kỳ báo cáo hoặc nhà máy. Dashboard chỉ tổng hợp dữ liệu chính thức đã được ghi nhận.</p>
            </section>
          ) : (
            <>
...
function ExecutiveKpi({ label, value, detail, tone }: {
  label: string
  value: string
  detail: string
  tone: string
...
function FinancialTrendChart({ points }: { points: ExecutiveTrendPoint[] }) {
  ...
  const x = (index: number) => padding.left + (values.length <= 1 ? plotWidth / 2 : index * plotWidth / (values.length - 1))
  const y = (value: number) => padding.top + (maximum - value) * plotHeight / span
  const line = (key: 'revenue' | 'expense' | 'profit') => values
    .map((point, index) => `${x(index)},${y(point[key])}`)
    .join(' ')
  const ticks = [0, 0.25, 0.5, 0.75, 1].map((ratio) => minimum + span * ratio)

  if (!values.length) return <div className="executive-panel-empty">Chưa có dữ liệu xu hướng tài chính.</div>
  ...
  return (
    <div className="executive-chart">
      ...
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Biểu đồ xu hướng doanh thu, chi phí và lợi nhuận">
        {ticks.map((tick) => (
          <g key={tick}>
            <line className="executive-grid-line" x1={padding.left} x2={width - padding.right} y1={y(tick)} y2={y(tick)} />
            <text className="executive-axis-text" x={padding.left - 10} y={y(tick) + 4} textAnchor="end">{compactMoney(tick)}</text>
          </g>
        ))}
        {minimum < 0 && <line className="executive-zero-line" x1={padding.left} x2={width - padding.right} y1={y(0)} y2={y(0)} />}
        ...
        {values.map((point, index) => (
          <g key={`${point.period}-${index}`}>
            {(['revenue', 'expense', 'profit'] as const).map((key) => (
              <circle key={key} className={`executive-dot ${key}`} cx={x(index)} cy={y(point[key])} r="3.5">
                <title>{`${point.period} · ${key === 'revenue' ? 'Doanh thu' : key === 'expense' ? 'Chi phí' : 'Lợi nhuận'}: ${money(point[key])}`}</title>
              </circle>
            ))}
            <text className="executive-axis-text" x={x(index)} y={height - 18} textAnchor="middle">{point.period}</text>
          </g>
        ))}
      </svg>
    </div>
...
function RiskList({ risks }: { risks: ExecutiveRisk[] }) {
  if (!risks.length) {
    return <div className="executive-panel-empty success">Không có rủi ro vượt ngưỡng trong kỳ báo cáo.</div>
  }
  return (
...
function DecisionList({ decisions }: { decisions: ExecutiveDecision[] }) {
  if (!decisions.length) {
    return <div className="executive-panel-empty success">Chưa có điểm nào cần Ban giám đốc ra quyết định.</div>
  }
  return (
    <div className="executive-decision-list">
      {decisions.map((decision, index) => (
        <article key={`${decision.relatedRiskCode}-${index}`} className="executive-decision">
          ...
          <div>
            <span>{domainLabels[decision.domain] ?? decision.domain} · {severityLabels[decision.priority] ?? decision.priority}</span>
            <b>{decision.title}</b>
