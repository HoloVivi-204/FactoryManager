...
public interface QualityReportStagingRepository extends JpaRepository<QualityReportStaging, Long> {
    ...
    @Query("select coalesce(sum(q.quantity), 0) from QualityReportStaging q where q.productionReportStaging.id = :reportId and q.active = true")
    Long sumActiveQuantityByReportId(@Param("reportId") Long reportId);

    @Query("select coalesce(sum(q.quantity), 0) from QualityReportStaging q where q.productionReportStaging.id = :reportId and q.active = true and q.id <> :excludedId")
