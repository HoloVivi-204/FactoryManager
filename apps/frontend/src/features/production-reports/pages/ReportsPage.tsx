...
import { DataTable, LoadingState, Panel } from '../../../shared/ui/ui'
import { number, percent } from '../../../shared/utils/format'
import { useApi } from '../../../shared/utils/useApi'
import type { ProductionReport, ProductionReportDetails } from '../../../shared/types'
        ...
        <DataTable
          ...
          columns={[
            ...
            { key: 'machineCode', label: 'Máy' },
            {
              key: 'actualQuantity',
              label: 'Thực tế',
              render: (row) => number(row.actualQuantity),
            },
            ...
            { key: 'oee', label: 'OEE', render: (row) => <b>{percent(row.oee)}</b> },
            {
              key: 'action',
              label: 'Thao tác',
              render: (row) => <button onClick={() => void openDetails(row)}>Chi tiết</button>,
            },
          ]}
        />
      </Panel>
  ...
  return (
    <div className="staging-detail-sections">
      <div className="review-stats">
        <span>
          Kế hoạch <b>{data.report.plannedQuantity}</b>
        </span>
        <span>
          Thực tế <b>{data.report.actualQuantity}</b>
        </span>
        <span>
          Đạt <b>{data.report.goodQuantity}</b>
        </span>
        <span>
          Lỗi <b>{data.report.defectQuantity}</b>
        </span>
        <span>
          OEE <b>{percent(data.report.oee)}</b>
        </span>
      </div>
